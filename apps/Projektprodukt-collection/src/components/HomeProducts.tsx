  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([e]) => {
        ...
        setInView(prev => (r >= 0.8 ? true : r <= 0.2 ? false : prev));
      },
    { threshold: [0, 0.2, 0.8, 1], rootMargin: "0px 0px -10% 0px" }
    );
    io.observe(el);
  ...
  return (
    <div ref={divRef} className="h-auto mt-[100px] first:mt-0 md:mt-0 md:h-screen w-full">
      <div className=" w-full h-full flex items-center justify-between">
        ...
        <div ref={ImgContainerRef} className="relative w-[min(55vw,600px)] overflow-hidden transform-gpu transition-[border-radius] duration-500 ease-out rounded-2xl
            hover:rounded-[15%]" onMouseMove={cursorMove} onMouseEnter={() => setCursorImgVisible(true)} onMouseLeave={() => setCursorImgVisible(false)}>
          ...
          { cursorImgVisible && !isTouch &&
            <img src={sunglassesCircle} alt="cursor" className="pointer-events-none absolute -translate-x-1/2 -translate-y-1/2 size-[100px]" style={{ top: cursorPos.y, left: cursorPos.x }}/>
          }
        </div>
